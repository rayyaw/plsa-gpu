...
ModelDataExtended loadModelFromFile() {
    ...
    ModelDataExtended model(count_data[0], count_data[1]);
...
void saveEmToFile(const EMstepExtended &data) {
    // Open files
...
void transposeDocumentCoverage(EMstepExtended &model) {
...
EMstepExtended runEm(ModelDataExtended &model, size_t num_topics, double prob_of_bg) {
    // Double buffering!
    EMstepExtended first = EMstepExtended(num_topics, model.document_count, model.vocab_size);
    EMstepExtended second = EMstepExtended(num_topics, model.document_count, model.vocab_size);
    ...
    for (size_t i = 0; i < MAXITER; i++) {
        ...
        if (update_first) {
            EMupdate::gpuUpdate(first, second, model, prob_of_bg, P_zdw_B_d, P_zdw_j_d, denoms_common_d);
        } else {
            EMupdate::gpuUpdate(second, first, model, prob_of_bg, P_zdw_B_d, P_zdw_j_d, denoms_common_d);
        }
        ...
        if (EMupdate::isConvergedGpu(first, second, coveragebuf_d, modelbuf_d)) {
            if (update_first) {
                transposeDocumentCoverage(first);
                break;
            } else {
